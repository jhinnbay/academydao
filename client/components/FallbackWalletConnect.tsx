...
export const FallbackWalletConnect: React.FC<FallbackWalletConnectProps> = ({ onConnectionChange }) => {
  const [isConnected, setIsConnected] = useState(false);
  const [address, setAddress] = useState<string>('');
  const [isConnecting, setIsConnecting] = useState(false);

  const checkTokenOwnership = async (walletAddress: string): Promise<boolean> => {
    try {
